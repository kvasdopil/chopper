...
- `supportsConeLoopMode`: identifies extrusion and cylinder modes that can use the cone/angled transition flag.
...
- `createLooseEdgeLoopExtrusionGeometry`: creates side-wall and perpendicular end-cap geometry for extrusion loop modes, with optional cone/angled transition geometry.
...
- `setLooseEdgeLoopCapCone`: updates one extrusion or cylinder loop, or exact paired loop's cone flag, and regenerates generated meshes.
...
- `handleLooseEdgeLoopConeChange`: updates the selected extrusion or cylinder loop's per-loop cone flag from the loop panel.
